...
export async function isDirective(req, res, next) {
  try {
    ...
    // Si el rol no es administrador, devolver un error 401
    if (rolUser === "administrador" || rolUser === "presidente" || rolUser === "secretario" || rolUser === "tesorero") 
    {
      next();
      return;
    }
    return res
        ...
        .json({
          message:
            "Error al acceder al recurso. Se requiere un rol de directiva para realizar esta acción.",
        });
  } catch (error) {
    res.status(500).json({ message: "Error interno del servidor", error });
  }
